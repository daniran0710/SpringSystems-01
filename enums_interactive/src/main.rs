...
fn perform_operation(operation: FileOperation) {
    match operation {
        FileOperation::List(path) => {
            let status = Command::new("ls").arg(&path).status().expect("Failed to execute ls");
        ...
        FileOperation::Display(path) => {
            let status = Command::new("cat").arg(&path).status().expect("Failed to execute cat");
        ...
        FileOperation::Create(path, content) => {
            ...
            let status = Command::new("sh").arg("-c").arg(&command).status().expect("Failed to create file");
        ...
        FileOperation::Remove(path) => {
            let status = Command::new("rm").arg(&path).status().expect("Failed to remove file");
